...
| [unity-skills-cli-bridge](./unity-skills-cli-bridge/SKILL.md) | Experimental Unity CLI bridge for a UnitySkills-bound project (opt-in via `Library/UnitySkills/cli_config.json`) — cold start with the Editor closed, headless test/run/build, exit codes, JSON/NDJSON contract, hard DO-NOT list. For general CLI operations use the official `unity-cli` skill. |
