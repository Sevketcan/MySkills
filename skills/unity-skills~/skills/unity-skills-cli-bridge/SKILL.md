---
name: unity-skills-cli-bridge
description: Use the Unity CLI only as the opt-in lifecycle bridge for a project bound to the Besty UnitySkills REST server—cold-start the Editor, run authorized headless tests/builds, and recover the REST connection. For general Unity CLI use, use the official unity-cli skill instead.
---
